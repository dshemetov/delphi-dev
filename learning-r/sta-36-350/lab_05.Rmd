...
pros.df %>%
  filter(lcp == min(lcp)) %>%
  summarize(min_lpsa = min(lpsa), max_lpsa = max(lpsa))
```
...
```{r q5a}
sprint.m.df %>% str()
sprint.m.df$Altitude <- sprint.m.df %>%
  pull(Time) %>%
  grepl("A$", .)
sprint.m.df <- sprint.m.df %>% mutate(Time = as.double(substr(Time, 0, 4)))
sprint.m.df %>% tibble()
```
...
```{r q5f}
dat.long <- dat.wide %>%
  pivot_longer(names_to = "City", values_to = "Time", -Name, values_drop_na = TRUE) %>%
  arrange(Name, City)
dat.long
identical(dat.long, dat.reduced)
```

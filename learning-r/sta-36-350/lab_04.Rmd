...
date: "Week of Tuesday September 21, 2021"
---
...
```{r q1a}
map(got_chars, "name") %>% as.character()
map_chr(got_chars, "name")
map_chr(got_chars, ~ .x$name)
map_chr(got_chars, \(x) x$name) # New lambda syntax in R 4.1
```
...
```{r q1b}
chars <- map_chr(got_chars, "name")
allegiances <- map_int(got_chars, ~ length(.x$allegiances))
chars[allegiances == max(allegiances)]
chars[allegiances == min(allegiances)]
```
...
```{r q1c}
got_chars[[1]]$tvSeries %in% paste("Season", 1:6)

a = 1
b = "hello"
paste("Season", a, "is", b)
sprintf("Season %d is %s", a, b)
glue::glue("Season {a} is {b}")
```
...
```{r q1d}
# Call <function> on <argument list>
# Maybe called apply
do.call(rbind, map(got_chars, ~ paste("Season", 1:6) %in% .$tvSeries))
rlang::inject(rbind(!!!map(got_chars, ~ paste("Season", 1:6) %in% .$tvSeries)))
# This doesn't quite do the thing, plus it makes multiple calls to rbind
reduce(map(got_chars, ~ paste("Season", 1:6) %in% .$tvSeries), rbind)
# Requires a list of dataframes
list_rbind(map(got_chars, ~ paste("Season", 1:6) %in% .$tvSeries))
# Kinda slow and looks strange
vctrs::vec_rbind(map(got_chars, ~ paste("Season", 1:6) %in% .$tvSeries))
```
...
```{r q1chal}
map_dfr(setNames(got_chars, chars), function(x) {
  seasons = paste("Season", 1:6)
  setNames(seasons %in% x$tvSeries, seasons)
}, .id="name")
```
...
```{r q2a}
got_df <- map_dfr(setNames(got_chars, chars), function(x) {
  c(born = x$born, died = x$died, gender = x$gender, culture = x$culture)
}, .id="name")
got_df %>% tail(n = 3)
got_df <- map_dfr(got_chars, `[`, c("born", "died", "gender", "culture"))
got_df %>% tail(n = 3)
```
...
```{r q2b}
got_df$alive = !is.na(got_df$died) & (got_df$died != "")
```
...
```{r q2c}
got_df %>% filter(culture == "Ironborn")
```
...
```{r q2d}
table(got_df$culture, got_df$gender)
```
...
```{r q2e}
got_df %>% group_by(culture) %>% summarize(n = sum(!alive)) %>% arrange(desc(n))
```
...
rio <- httr::GET(
  "https://www.stat.cmu.edu/~ryantibs/statcomp/data/rio.csv",
...
```{r q3a}
rio %>% group_by(nationality) %>% summarize(n = n()) %>% head(n = 10)
rio %>% group_by(nationality) %>% summarize(n = n()) %>% filter(n == max(n))
```

- **3b.** Using `group_by()`, `summarize()`, and `filter()`, compute which country had the most numnber of total medals and many that was. 

```{r q3b}
rio %>% group_by(nationality) %>% summarize(total_medals = sum(gold) + sum(silver) + sum(bronze)) %>% filter(total_medals == max(total_medals))
```
...
```{r q3c}
rio %>% group_by(nationality) %>% summarize(n = n(), total_medals = sum(gold) + sum(silver) + sum(bronze)) %>% filter(total_medals == 0) %>% filter(n == max(n))
```
...
```{r q3d}
rio %>% group_by(sport, sex) %>% summarize(avg_weight = mean(weight, na.rm = TRUE)) %>% ungroup() %>% group_by(sex) %>% summarize(sport = sport[which.max(avg_weight)], avg_weight = max(avg_weight))
```
...
sprint.m.df <- httr::GET(
  "http://www.stat.cmu.edu/~ryantibs/statcomp/data/sprint.m.txt",
  config = httr::config(ssl_verifypeer = 0L, ssl_verifyhost = 0L)
) %>%
  httr::content(as = "text", encoding = "UTF-8") %>%
  readr::read_table() %>%
  as.data.frame()
sprint.w.df <- httr::GET(
  "http://www.stat.cmu.edu/~ryantibs/statcomp/data/sprint.w.txt",
  config = httr::config(ssl_verifypeer = 0L, ssl_verifyhost = 0L)
) %>%
  httr::content(as = "text", encoding = "UTF-8") %>%
  readr::read_table() %>%
...
sprint.m.df %>% head(n = 3)
sprint.m.df %>% tail(n = 3)
```

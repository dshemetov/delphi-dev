...
dplyr::case_when(x.vec %% 2 == 0 ~ x.vec + 10, .default = x.vec)
# Doesn't destroy Dates in output, does type checking (tries not to destroy other types too), has sanity checking, has an NA handling branch
dplyr::if_else(x.vec %% 2 == 0, x.vec + 10, x.vec)

# Example of ifelse destroying the factor type -> integer
...
x.list[c(1, 3)]
x.list[[2]][1:10]
# Automatic NA pad if slicing out of range
x.list[[1]][1:10]
```
...
```{r}
pros.dat <- read.table("/home/dskel/Documents/Code/Delphi/learning-r/sta-36-350/pros.dat", header = TRUE)
pros.dat
```
...
```{r q2e}
pros.dat[, "lweight"] - pros.dat[, "lcavol"]
```
...
```{r q2f}
out <- cbind(pros.dat, pros.dat[, "lweight"] - pros.dat[, "lcavol"])
colnames(out)
out2 <- cbind(out, pros.dat[, "lweight"])
colnames(out2)
out2[, 11]
colnames(out)
out
# This doesn't work (praise god)
list("" = 5)
# This does work (oh no 💀)
list("a" = 5, "a" = 7)["a"]
```

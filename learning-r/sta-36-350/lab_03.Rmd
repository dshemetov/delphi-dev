...
date: "Week of Tuesday September 14, 2021"
---
...
```{r q1c}
state.df[c("Center.x", "Center.y")] <- state.center[c("x", "y")]
head(state.df)
```
...
```{r q1d}
sub.state.df.1 <- subset(state.df, Center.x < -100)
sub.state.df.2 <- state.df[state.df$Center.x < -100, ]
identical(sub.state.df.1, sub.state.df.2)
```
...
```{r q1e}
subset(state.df, Center.x < -100 & Murder > 9.0) %>% slice_max(Life.Exp)
```
...
```{r}
pros.dat <- httr::GET(
  "https://www.stat.cmu.edu/~ryantibs/statcomp/data/pros.dat",
...
pros.dat <-
  read.table("https://www.stat.cmu.edu/~ryantibs/statcomp/data/pros.dat")
```
...
duplicated(rio$id)
anyDuplicated(rio$id)

any(is.na(rio))
rio[rowSums(is.na(rio)), ]
length(rowSums(is.na(rio)))

rio %>%
  dplyr::count(name, sport) %>%
  dplyr::distinct(n)

rio %>%
  na.omit()

purrr::map_int(rio, function(col) sum(is.na(col)))
```
...
rio %>% dplyr::count(nationality)
rio %>%
  dplyr::count(nationality) %>%
  dplyr::slice_max(n)
table(rio$nationality)
table(rio$nationality) %>% which.max()
```
...
```{r q3c}
vapply(rio[, c("gold", "silver", "bronze")], sum, numeric(1))
...
```{r q3e}
rio[duplicated(rio$name),]
rio[rio$name == "Qian Li",]

rio %>% dplyr::count(name, date_of_birth) %>% dplyr::filter(n > 1)

tapply(rio %>% mutate(total = gold + silver + bronze) %>% pull(total), rio$nationality, sum)
```
